...
if os.path.exists(final_model_filepath):
    ...
    try:
        initial_epoch = model.history.epoch[-1] + 1 if model.history else 0  # Adjust epoch tracking if history exists
    except:
        initial_epoch = 0
else:
...
checkpoint_filepath = 'checkpoints/model_epoch_{epoch:02d}_val_acc_{val_accuracy:.2f}.keras'
